...
\draw[pencildraw,draw,-latex] (src) -- (two);

\draw [<->]  (0:1)   arc (0:60:1)   node[midway,above right]{$\beta$};
...
\node at (7,4.5) {$\color{blue!70}\pi_{i,{dst}}^1$};
\node at (5,2) {$\color{blue!70}\pi_{i,{dst}}^2$};
 
	\node at (7,2.4) {$\color{red}\pmb{\lambda}_1$};
	\node at (6.8,3.35) {$\color{red}\pmb{\lambda}_2$};
